    ...
    for(var i = 0; i < this.game.entities.length; i++) {
        ...
        if(ent.name === "ShipProjectile") {
            ...
            if(this.xMid > ent.xMid) {
            	this.xMid = this.xMid - (Math.sqrt((delta * delta) * (dX * dX)))*this.speed;
            } else if(this.xMid < ent.xMid) {
            	this.xMid = (Math.sqrt((delta * delta) * (dX * dX)))* this.speed + this.xMid;
            }

            if(this.yMid > ent.yMid) {
            	this.yMid = this.yMid - (Math.sqrt((delta * delta) * (dY * dY)))* this.speed;
            } else if(this.yMid < ent.yMid) {
            	this.yMid = (Math.sqrt((delta * delta) * (dY * dY)))*this.speed + this.yMid;
            }
...
TheShip.prototype.draw = function () {
	...
	this.reticleAnimation.drawFrame(this.game.clockTick, this.ctx,
								   ...
								   (this.game.mouseY - (this.pHeight * 0.25 / 2) - 1), 0);
